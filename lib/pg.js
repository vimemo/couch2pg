...
export default class Pg {
  ...
  /**
   * Abstraction for replication target db.
   *
   * @constructor
   *
   * @param {String} url
   *   postgres url
   * @param {String} dbid
   *   source database id that will replicate to this pg.
   */
   constructor(url=throwIfMissing(), dbid=throwIfMissing()) {
    this.url = url
  ...
  async docs(sorted=false) {
    ...
    return sorted ? R.sortBy(R.prop('_id'), rows) : rows
  }

  async insert(docs) {
    log.debug(`Inserting ${docs.length} results into postgresql`)
    let sql = format(stmts.insert, docs.map(row => [row.doc]))
